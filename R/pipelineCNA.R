#' SCEVAN: R package that automatically classifies the cells in the scRNA data
#' by segregating non-malignant cells of tumor microenviroment from the
#' malignant cells. It also infers the copy number profile of malignant cells,
#' identifies subclonal structures and analyses the specific and shared
#' alterations of each subpopulation.
#'
#' The SCEVAN package
#'
...
#' @importFrom graphics mtext par plot.new rect text title
#' @importFrom stats approx as.dendrogram as.dist cor cor.test cutree density
#' @importFrom stats dnorm hclust median order.dendrogram p.adjust pnorm 
#' @importFrom stats predict reorder rnorm runmed sd smooth.spline t.test 
#' @importFrom utils read.csv read.table write.table
## usethis namespace: end
NULL

#' pipelineCNA Executes the entire SCEVAN pipeline that classifies tumour and
#' normal cells from the raw count matrix, infer the clonal profile of cancer
#' cells and looks for possible sub-clones in the tumour cell matrix
#' automatically analysing the specific and shared alterations of each subclone
#' and a differential analysis of pathways and genes expressed in each subclone.
#'
#' @param count_mtx Raw count matrix with genes on rows (both Gene Symbol or
#' Ensembl ID are allowed) and cells on columns.
#' @param sample Sample name to save results (optional)
#' @param par_cores Number of cores to run the pipeline (optional - default 20)
#' @param norm_cell Vector of normal cells if the classification is already
#' known and you are only interested in the clonal structure (optional)
#' @param SUBCLONES Boolean value TRUE if you are interested in analysing the
#' clonal structure and FALSE if you are only interested in the classification
#' of malignant and non-malignant cells (optional - default TRUE)
#' @param beta_vega Specifies beta parameter for segmentation, higher beta for
#' more coarse-grained segmentation. (optional - default 0.5)
#' @param ClonalCN Get clonal CN profile inference from all tumour cells
#' (optional)
#' @param plotTree Plot Phylogenetic tree (optional - default FALSE)
#' @param AdditionalGeneSets list of additional signatures of normal cell types
#' (optional)
#' @param SCEVANsignatures FALSE if you only want to use only the signatures
#' specified in AdditionalGeneSets (default TRUE)
#' @param organism Organism to be analysed (optional - "mouse" or "human" -
#' default "human")
#' @param ngenes_chr Minimum number of genes expressed on chromosome (optional -
#' default 5)
#' @param perc_genes Minimum percentage gene expressed in each cell (optional -
#' default 10)
#'
#' @return
#' @export
pipelineCNA <- function(count_mtx, sample = "", par_cores = 20, norm_cell = NULL, SUBCLONES = TRUE, beta_vega = 0.5, ClonalCN = TRUE, plotTree = TRUE, AdditionalGeneSets = NULL, SCEVANsignatures = TRUE, organism = "human", ngenes_chr = 5, perc_genes = 10) {
  ...
  if (ClonalCN) {
    getClonalCNProfile(res_class, res_proc, sample, par_cores,
      organism = organism
    )
  }
  ...
  if (SUBCLONES) {
    res_subclones <- subcloneAnalysisPipeline(
      res_proc$count_mtx, res_class, res_proc, mtx_vega, sample, par_cores, classDf, beta_vega, plotTree, organism
    )
