...
top30classification <- function(NES, pValue, FDR, pval_filter, fdr_filter, pval_cutoff, nes_cutoff, nNES) {
  ...
  if (length(conf_norm_cells) > 0) {
    conf_norm_cells <- sort(conf_norm_cells, decreasing = TRUE)[
      seq_len(min(30, length(conf_norm_cells)))
    ]
...
#' getConfidentNormalCells Get at most top 30 confident normal cells from count
#' matrix.
#'
...
getConfidentNormalCells <- function(mtx, sample = "", par_cores = 20, AdditionalGeneSets = NULL, SCEVANsignatures = TRUE, organism = "human") {
  ...
  ssMwwGst(
    geData = mtx, geneSet = geneSet,
    ncore = par_cores,
    minLenGeneSet = 5,
    filename = paste0("./output/", sample, "_confidentNormal"),
    standardize = TRUE
  )
  load(paste0("./output/", sample, "_confidentNormal_MWW.RData"))
  norm.cell.names <- top30classification(
    NES = NES, FDR = FDR, pValue = pValue,
    fdr_filter = TRUE, pval_filter = TRUE,
    pval_cutoff = 1 * 10^(-10), nes_cutoff = 1.0, nNES = 1
  )
