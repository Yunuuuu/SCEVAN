...
computeCNAmtx <- function(count_mtx, breaks, par_cores = 20, segmAlt) {
  ...
  funcCNA <- function(z) {
    x <- numeric(n)
    for (i in seq_along(startBreaks)) {
      if (segmAlt[i]) {
  ...
  if (Sys.info()["sysname"] == "Windows") {
    ...
    seg.test <- parallel::parLapply(cl, seq_len(ncol(count_mtx)), funcCNA)

    parallel::stopCluster(cl)
  } else {
    seg.test <- parallel::mclapply(seq_len(ncol(count_mtx)), funcCNA, mc.cores = par_cores)
  }

  # seg.test <- parallel::mclapply(seq_len(ncol(count_mtx)), function(z){
  #   x<-numeric(n)
  #   for (i in 1:(length(breaks)-1)){
  #
  #     if(segmAlt[i]){
...
#' classifyCluster Classify the two major clusters of CNA matrix on the basis of
...
#' classifyTumorCells Classify tumour and normal cells from the raw count
#' matrix, using normal cells in the matrix or by subtracting a synthetic
#' baseline from the matrix if there are no normal cells in the matrix.
#'
#' @param count_mtx raw count matrix
#' @param annot_mtx matrix containing the annotations of the genes (rows: genes,
...
#' @param SEGMENTATION_CLASS Boolean value to perform segmentation before
...
#' more coarse-grained segmentation. (default 0.5)
#'
#' @export
