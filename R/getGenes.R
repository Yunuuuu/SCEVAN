getGenes <-
  function(out_file_name, segmentation, pval_threshold, baf, ensembl_dataset,
    ...
    message(
      "Downloading Gene Information from \"", ensembl_dataset,
      "\" Database"
    )
    ...
    for (chr in chromosomes) {
      ...
      == as.character(chr)), ]
      if (!is.matrix(curr_seg)) {
        curr_seg <- t(as.matrix(curr_seg))
      }
      for (j in 1:nrow(curr_seg)) {
        curr_reg <- curr_seg[j, ]
        if (min(c(
          as.numeric(curr_reg[6]), as.numeric(curr_reg[7]),
          as.numeric(curr_reg[8])
        )) <= pval_threshold) {
          genes_details <- getBM(
            attributes = c(
              "ensembl_gene_id",
    ...
    colnames(del_genes) <- c(
      "Ensembl Gene ID", "External Gene ID",
    ...
    if (nrow(del_genes) > 0) {
      ...
      del_genes <-
        del_genes[order(as.numeric(substr(
          del_genes[, 10], 1,
          nchar(del_genes[, 10]) - 1
        )), decreasing = TRUE), ]

      write.table(del_genes, delfile,
        sep = "\t", col.names = TRUE,
        row.names = FALSE, eol = "\n", quote = FALSE
      )
    ...
    if (nrow(amp_genes) > 0) {
      ...
      amp_genes <-
        amp_genes[order(as.numeric(substr(
          amp_genes[, 10], 1,
          nchar(amp_genes[, 10]) - 1
        )), decreasing = TRUE), ]

      write.table(amp_genes, ampfile,
        sep = "\t", col.names = TRUE,
        row.names = FALSE, eol = "\n", quote = FALSE
      )
    ...
    if (baf == TRUE) {
      if (nrow(loh_genes) > 0) {
        ...
        loh_genes <-
          loh_genes[order(as.numeric(substr(
            loh_genes[, 10], 1,
            nchar(loh_genes[, 10]) - 1
          )), decreasing = TRUE), ]

        write.table(loh_genes, lohfile,
          sep = "\t", col.names = TRUE,
          row.names = FALSE, eol = "\n", quote = FALSE
        )
