vegaMC_R <- function(mtx, output_file_name = "output",
  ...
  segmentation <- read.table(output_file_name,
    sep = "\t",
  ...
  f_l <-
    as.numeric(segmentation[, 9]) * abs(
      as.numeric(segmentation[, 13])
    ) / as.numeric(segmentation[, 12])
  f_g <-
    as.numeric(segmentation[, 10]) * abs(
      as.numeric(segmentation[, 14])
    ) / as.numeric(segmentation[, 12])
  f_loh <-
    as.numeric(segmentation[, 11]) * abs(
      as.numeric(segmentation[, 15])
    ) / as.numeric(segmentation[, 12])
  ...
  colnames(segmentation)[12:18] <- c(
    "Probe Size", "Loss Mean", "Gain Mean",
  ...
  write.table(segmentation, output_file_name,
    sep = "\t", row.names = FALSE,
