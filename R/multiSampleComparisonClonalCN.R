...
multiSampleComparisonClonalCN <- function(listCountMtx, listNormCells = NULL, analysisName = "all", organism = "human", par_cores = 20, plotTree = TRUE) {
  ...
  names(sampleAlterList) <- paste0(
    names(listCountMtx),
    "_subclone", seq_along(names(listCountMtx))
  )
  ...
  if (length(names(listCountMtx)) > 2 && plotTree) {
    plotCloneTreeNew(names(listCountMtx),
      CLONAL_MULTI = TRUE, analysisName = analysisName
    )
  }

  for (i in seq_along(names(listCountMtx))) {
    names(diffList) <- gsub(
      paste0("subclone", i),
      names(listCountMtx)[i], names(diffList)
    )
  }
  ...
  save(outputAnalysis,
    file = paste0("./output/", analysisName, "_outputAnalysis.RData")
  )
