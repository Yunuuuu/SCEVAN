...
heatmap.3 <- function(x,
  ...
  if (is.function(col)) col <- col(ncol)
  ...
  axis(1, extr_chr[2:(totChr + 1)] - diff(extr_chr) / 2,
    labels = 1:totChr, las = 1, line = 0.2, tick = 0,
    cex.axis = cexCol, gap.axis = 0
  )

  if (!is.null(labels_gene)) {
    for (i in seq_len(nrow(labels_gene))) {
      axis(3, labels_gene[i, 1],
        labels = labels_gene[i, 2], las = 1, line = 0.2, tick = TRUE,
...
plotTSNE <- function(raw_count_mtx, CNAmat, filt_genes, tum_cells, clustersSub, samp) {
  ...
  pp <- ggplot(df, aes(.data$x, .data$y, colour = .data$CellType)) +
    geom_point() +
    theme_bw() +
    scale_color_manual(
  ...
  if (length(unique(clustersSub)) > 0) {
    ...
    pp <- ggplot(df, aes(.data$x, .data$y, colour = .data$Subclones)) +
      geom_point() +
      theme_bw() +
...
plotCNsubclones <- function(samp) {
  ...
  extr_chr <- append(1, extr_chr)
  axis(1, extr_chr[2:23] - diff(extr_chr) / 2,
    labels = 1:22, las = 1, line = 0.2, tick = 0,
    cex.axis = 1.0, gap.axis = 0
  )

  legend("topright",
    inset = c(0, 0), legend = c("Shared Gain", "Shared Loss", "Subclone 1", "Subclone 2"),
...
plotCNAline <- function(segmList, segmListSpec, samp, nSub, colors_samp = NULL) {
  ...
  heatmap.3(t(do.call(cbind, lapply(df, function(x) x$Mean))),
    Rowv = FALSE, Colv = FALSE, dendrogram = "none", chr_lab = 1:22, keysize = 1, density.info = "none", trace = "none",
...
plotConsensusCNA <- function(samp, nSub, organism = "human", pathOutput = "./output/", colors_samp = NULL) {
  ...
  png(paste("./output/", samp, "consensus.png", sep = ""), height = 750, width = 2850, res = 180)
  heatmap.3(t(do.call(cbind, lapply(df, function(x) x$Mean))),
    Rowv = FALSE, Colv = FALSE, dendrogram = "none", chr_lab = df_VEGAchr$y, keysize = 1, density.info = "none", trace = "none",
    cexRow = 3.0, cexCol = 2.0, cex.main = 3.0, cex.lab = 3.0,
    ColSideColors = chr1,
    symm = F, symkey = F, symbreaks = T, cex = 3.0, main = paste("Sample:", samp), cex.main = 4, margins = c(10, 10), key = FALSE,
    notecol = "black", col = my_palette, RowSideColors = cells
...
plotCNAlineOnlyTumor <- function(samp) {
  ...
  png(paste("./output/", samp, "consensus.png", sep = ""), height = 750, width = 2850, res = 180)
  heatmap.3(t(cbind(dfL$Mean, dfL$Mean)),
    Rowv = FALSE, Colv = FALSE, dendrogram = "none", chr_lab = df_VEGAchr$y, keysize = 1, density.info = "none", trace = "none",
    cexRow = 3.0, cexCol = 2.0, cex.main = 3.0, cex.lab = 3.0,
    ColSideColors = chr1,
    symm = F, symkey = F, symbreaks = T, cex = 3.0, main = paste("Sample:", samp), cex.main = 4, margins = c(10, 10), key = FALSE,
